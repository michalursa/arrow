...
void Hashing::avalanche(util::CPUInstructionSet instruction_set, uint32_t num_keys,
                        uint32_t* hashes) {
...
void Hashing::helper_tails(util::CPUInstructionSet instruction_set, uint32_t num_keys,
                           uint32_t key_length, const uint8_t* keys, uint32_t* hash) {
  uint32_t processed = 0;
#if defined(ARROW_HAVE_AVX2)
  if (instruction_set == util::CPUInstructionSet::avx2) {
...
void Hashing::hash_fixed(util::CPUInstructionSet instruction_set, uint32_t num_keys,
  ...
  helper_stripes(instruction_set, num_keys, length_key, keys, hashes);
  if ((length_key % 16) > 0 && (length_key % 16) <= 8) {
    helper_tails(instruction_set, num_keys, length_key, keys, hashes);
  }
  avalanche(instruction_set, num_keys, hashes);
}
...
void Hashing::hash_varlen(util::CPUInstructionSet instruction_set, uint32_t num_rows,

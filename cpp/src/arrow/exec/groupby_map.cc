...
Status SwissTable::map(const int num_keys, const uint32_t* hashes,
  ...
  if (num_inserted_ > 0 && precomputed_available()) {
    ...
    // TODO: explain why entering this section with zero rows to process slows down
    // performance almost 2x for 3 groups
...
#else
    for (int i = 0; i < num_keys; ++i) {
      slot_ids[i] = (hashes[i] >> (32 - log_blocks_)) * 8;
      ids[i] = i;
    }
    num_ids = num_keys;
#endif
...
Status SwissTable::grow_double() {
  ...
  uint64_t group_id_mask_after = ~0ULL >> (64 - num_group_id_bits_after);

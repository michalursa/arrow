...
void Hashing::hash_varlen_avx2(uint32_t num_rows, const uint32_t* offsets,
                               ...
                               uint32_t* hashes) {
  const __m128i sequence = _mm_setr_epi64x(0x0706050403020100ULL, 0x0f0e0d0c0b0a0908ULL);
  ...
  for (uint32_t i = num_rows - num_rows % unroll; i < num_rows; ++i) {
    ...
    uint32_t acc = _rotl(temp_buffer_base[0], 1) + _rotl(temp_buffer_base[1], 7) +
                   _rotl(temp_buffer_base[2], 12) + _rotl(temp_buffer_base[3], 18);
